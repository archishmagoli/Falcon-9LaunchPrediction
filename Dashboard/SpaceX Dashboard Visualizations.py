...
spacex_df = pd.read_csv("Dashboard\spacex_launch_dash.csv")
...
app.layout = html.Div(children=[html.H1('SpaceX Launch Records Dashboard',
                                ...
                                html.Div(dcc.Graph(id='success-pie-chart', figure=px.pie(spacex_df[spacex_df['class'] == 1], values='class', names='Launch Site', title='Successful Launches'))),
                                html.Br(),
...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        fig=px.pie(spacex_df[spacex_df['class'] == 1], names='Launch Site', title='Successful Launches')
        fig.update_layout()
        return fig
    else:
        # return the outcomes piechart for a selected site
        filtered_df = spacex_df[spacex_df['Launch Site'] == entered_site]
        fig = px.pie(filtered_df, names=filtered_df['class'].map({0:'Failure', 1:'Success'}), title='Success Rate of Launches for site ' + entered_site)
        fig.update_layout()
        return fig
